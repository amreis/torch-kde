...
class TestKDE(unittest.TestCase):
    ...
    def test_integral(self):
        ...
        for kernel_str, bandwidth, dim, weights in product(
            SUPPORTED_KERNELS, BANDWIDTHS, DIMS, WEIGHTS
        ):
            ...
            meshgrid = torch.meshgrid(
                *ranges, indexing="ij"
            )  # 'ij' indexing for Cartesian coordinates
    ...
    def test_vmf_integral(self):  # von-Mises-Fisher must be tested differently from other kernels
        for bandwidth, dim, weights in product(BANDWIDTHS, DIMS, WEIGHTS):
            if dim == 1 or isinstance(bandwidth, str):
                # Skip the von-mises-fisher kernel, must be handled differently
                # as it is not defined in the same way as the other kernels
                continue
            X = sample_from_gaussian(dim, N1)
            ...
            mesh_samples = mesh_samples / mesh_samples.norm(
                dim=1, keepdim=True
            )  # project onto sphere
    ...
    def test_diffble(self, bandwidth=torch.tensor(1.0), eps=1e-07):
        ...
        for kernel_str, dim in product(SUPPORTED_KERNELS, DIMS):
            ...
            X = sample_from_gaussian(dim, N1).to(
                torch.float64
            )  # relevant for the gradient check to convert to double
            ...
            self.assertTrue(
                gradcheck(lambda X_: fnc(X=X_), (X,), raise_exception=False, eps=eps),
            ...
            if not kernel_str == "von-mises-fisher":  # normalization required
                ...
                self.assertTrue(
                    gradcheck(
                        lambda bandwidth_: fnc(bandwidth=bandwidth_),
                        (bandwidth,),
            ...
            self.assertTrue(
                gradcheck(
                    lambda X_new_: fnc(X_new=X_new_), (X_new,), raise_exception=False, eps=eps
                ),
                f"""Kernel {kernel_str}, for dimensionality {str(dim)} is not differentiable w.r.t evaluation data.""",
            )

    def test_sampling_adheres_to_weights(
        self,
    ):
        ...
        pi = torch.tensor(
            COMPONENT_WEIGHTS
        )  # 90 % for component 1, 0.5 % for component 2 (does not need to sum to 1)
        ...
        covs = torch.stack(
            [cov1, cov2]
        )  # Shape: [n_components, event_shape, event_shape] = [2, 2, 2]

        component_distribution = dist.multivariate_normal.MultivariateNormal(
            loc=locs, covariance_matrix=covs
        )
        ...
        gmm = dist.MixtureSameFamily(
            mixture_distribution=mixing_distribution, component_distribution=component_distribution
        )
        ...
        kde = torchkde.KernelDensity(
            bandwidth=0.5, kernel="gaussian"
        )  # create kde object with isotropic bandwidth matrix
        kde.fit(
            torch.concat((X1, X2), dim=0), sample_weight=torch.concat((weights1, weights2), dim=0)
        )  # fit kde to weighted data

        samples_from_kde = kde.sample(N1)
        samples_from_gmm = gmm.sample((N1,))

        component_1_fraction_kde = (
            torch.count_nonzero(torch.where(samples_from_kde[:, 0] > THRESHOLD, 1.0, 0.0)) / N3
        )
        component_1_fraction_gmm = (
            torch.count_nonzero(torch.where(samples_from_gmm[:, 0] > THRESHOLD, 1.0, 0.0)) / N3
        )

        self.assertTrue(
            1.0 - TOLERANCE
            < (component_1_fraction_kde / component_1_fraction_gmm)
            < 1.0 + TOLERANCE,
            "Component weights must be considered on sampling.",
        )
